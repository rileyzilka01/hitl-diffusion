...
expert_data_path = '/home/serg/projects/png_vision/data/bowl_in_bowl'
...
for demo_dir in demo_dirs:
    ...
    for step_idx in tqdm.tqdm(range(len(demo_timesteps))):
        ...
        robot_state = np.hstack(state_info['ee_position'] + state_info['ee_orientation'] + list(state_info['joints']['position'])[:7])
        if prev_ee_pos is None:
            action = [0, 0, 0, 0, 0, 0]
        else:
            prev_ori = R.from_euler('xyz', state_info['ee_orientation'], degrees=True)
            curr_ori = R.from_euler('xyz', prev_ee_ori, degrees=True)
